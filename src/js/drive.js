...
(function($) {
  ...
  jQuery.extend(App.Drive.prototype, {
    ...
    setState: function(state) {
      ...
      self.logging.info(
        "setState: current = " + self.state + ", new = " + state
      );
    ...
    handleInvalidToken: function() {
      ...
      return self.refreshToken().fail(function() {
        self.logging.info(
          "Failed to refresh token, setting state to unauthorized"
        );
    ...
    authURL: function() {
      ...
      var url =
        ...
        "?redirect_uri=" +
        encodeURIComponent(window.config.redirect_uri) +
        "&response_type=code" +
        "&client_id=" +
        window.config.client_id +
        "&scope=" +
        window.config.scopes.join(" ") +
    ...
    refreshToken: function() {
      ...
      self
        .deferredProperty(App.Drive.Property.REFRESH_TOKEN)
          ...
          .then(function(token) {
            ...
            var retrievePageOfFiles = function(nextPageToken) {
              var params = {
                maxResults: "100",
                q:
                  "(fullText contains '*.gb' or fullText contains '*.gbc') and trashed = false and mimeType = 'application/octet-stream'",
                access_token: token
              };
